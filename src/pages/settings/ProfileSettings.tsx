    ...
    const fetchRoles = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('user_roles').select('id, name');
        if (error) {
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="space-y-1">
          <h4 className="text-sm font-medium text-left">Profile Picture</h4>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsCropperOpen(true)} disabled={isLoading}>
              {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
              Change avatar
            </Button>
            {avatarUrl && <Button variant="outline" size="sm" onClick={async () => {
            try {
              setIsLoading(true);
              const {
                error
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          field
                ...
                <FormControl>
                  <Textarea {...field} placeholder="Tell us a little about yourself" className="resize-none h-20" />
                </FormControl>
                <FormDescription className="text-left">
          ...
          field
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {allRoles.length > 0 ? allRoles.map(role => <SelectItem key={role.id} value={role.name}>
                          {role.name}
                        </SelectItem>) : <>
      ...
      <Dialog open={isCropperOpen} onOpenChange={setIsCropperOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <ImageUploadCropper aspectRatio={1} onImageCropped={handleImageCropped} onCancel={() => setIsCropperOpen(false)} isUploading={isLoading} />
        </DialogContent>
      </Dialog>
    </div>;
}